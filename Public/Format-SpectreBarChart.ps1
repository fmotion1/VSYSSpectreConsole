    ...
    process {
        if ($Data -is [array]) {
            foreach ($dataItem in $Data) {
                $barChart = [BarChartExtensions]::AddItem($barChart, $dataItem.Label, $dataItem.Value, ($dataItem.Color | Get-SpectreColor))
            }
        } else {
            $barChart = [BarChartExtensions]::AddItem($barChart, $Data.Label, $Data.Value, ($Data.Color | Get-SpectreColor))
        }
    }
    end {
