...
function Read-SpectreConfirm {
    ...
    # This is much fiddlier but it exposes the ability to set the color scheme. The
    # confirmation prompt is just a textprompt with two choices hard coded to y/n:
    ...
    # This is how I added the default colors with Set-SpectreColors so you don't have
    # to keep passing them through and get a consistent TUI color scheme
    $confirmationPrompt.DefaultValueStyle = [Style]::new($script:DefaultValueColor)
    $confirmationPrompt.ChoicesStyle = [Style]::new(($Color | Get-SpectreColor))
