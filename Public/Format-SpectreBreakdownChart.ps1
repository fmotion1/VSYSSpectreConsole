...
function Format-SpectreBreakdownChart {
    ...
    param (
        ...
        [array] $Data,
        [ValidateScript({ 
            $_ -gt 0 -and $_ -le [console]::BufferWidth 
        }, ErrorMessage = "Value '{0}' is invalid. Cannot be negative or exceed console width.")]
    ...
    process {
        if($Data -is [array]) {
            foreach($dataItem in $Data) {
                [BreakdownChartExtensions]::AddItem($chart, $dataItem.Label, $dataItem.Value, ($dataItem.Color | Get-SpectreColor)) | Out-Null
            }
        } else {
            [BreakdownChartExtensions]::AddItem($chart, $Data.Label, $Data.Value, ($Data.Color | Get-SpectreColor)) | Out-Null
        }
    }
    end {
